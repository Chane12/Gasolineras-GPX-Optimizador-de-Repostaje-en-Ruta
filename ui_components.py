...
def render_welcome_screen(is_mobile: bool = False):
    ...
    if is_mobile:
        ...
        st.markdown(
            "<div style='text-align:center; padding: 1rem 0;'><span style='font-size:3rem;'>🛣️⛽</span></div>",
        ...
        with col_left:
            ...
            for col, (icon, title, desc) in zip(pill_cols, pills):
                with col, st.container(border=True):
                    st.markdown(
                        f"<div style='text-align:center; font-size:1.6rem;'>{icon}</div>", unsafe_allow_html=True
                    )
                    st.markdown(
                        f"<strong style='display:block; text-align:center;'>{title}</strong>",
                        unsafe_allow_html=True,
                    )
                    st.caption(desc)
            st.markdown("")
...
def render_config_summary(
    ...
    origen_txt: str = "",
    destino_txt: str = "",
    using_gpx: bool = False,
    using_demo: bool = False,
):
    """Shows an active-config summary badge in the sidebar after the pipeline has run."""
    # Sentinel: Sanitize user inputs to prevent XSS when rendering HTML
    combustible_safe = html.escape(combustible) if combustible else ""
    origen_safe = html.escape(origen_txt.strip().title()) if origen_txt else ""
    destino_safe = html.escape(destino_txt.strip().title()) if destino_txt else ""
    ...
    if using_demo:
        ...
        ruta_str = f"📑 {origen_safe} → {destino_safe}"
    else:
    ...
    st.markdown(
        ...
        {ruta_str}<br>
        ⛽ {combustible_safe} · 📍 ±{radio_km} km · 🏆 Top {top_n}<br>
...
def render_metric_cards(
    precio_top_min: float, ahorro_vs_caro: float, total_mostradas: int, total_zona: int, radio_km: int, fuel_column: str
):
...
def render_station_cards(df_show, precio_col_label: str, station_coords: list, mis_paradas: list):
    ...
    for i, (_, row) in enumerate(df_cards.iterrows()):
        ...
        ya_en_plan = any(p.get("_geom_x") == coords_x and p.get("_geom_y") == coords_y for p in mis_paradas)
        ...
        with st.container(border=True):
            ...
            with col_precio:
                st.markdown(
                    ...
                    + (
                        f"<div style='text-align:right; font-size:0.8rem; color:#4CAF50;'>{ahorro_str}</div>"
                        if ahorro_str
                        else ""
                    ),
                    unsafe_allow_html=True,
            ...
            for t in tramos_peligro:
                with st.container(border=True):
                    ...
                    if autonomia_km > 0:
                        ...
                        if t["nivel"] == "critico":
                            st.error(
                                f"⚠️ Supera tu autonomía en {t['gap_km'] - autonomia_km:.1f} km — Repostar OBLIGATORIAMENTE antes de este tramo."
                            )
                        elif t["nivel"] == "atencion":
                            st.warning(
                                f"⚡ Este tramo de {t['gap_km']:.0f} km consumirá el {pct_real:.0f}% de tu tanque. Entra con suficiente combustible."
                            )
                        else:
                            if t["gap_km"] >= 60:
                                st.info(
                                    f"ℹ️ **Tramo de {t['gap_km']:.0f} km sin gasolineras.** Para tu vehículo solo supone un {pct_real:.0f}% de la autonomía total. Tienes margen seguro."
                                )
                    else:
                        if t["gap_km"] >= 100:
                            st.error(
                                f"🚨 **Tramo muy largo ({t['gap_km']:.0f} km sin gasolineras)** — Asegúrate de llevar un nivel alto de combustible antes de entrar."
                            )
                        elif t["gap_km"] >= 60:
                            st.warning(
                                f"⚠️ **Tramo largo ({t['gap_km']:.0f} km sin gasolineras)** — Revisa tu nivel de combustible con antelación."
                            )
    else:
