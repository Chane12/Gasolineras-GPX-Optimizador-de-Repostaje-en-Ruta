...
def render_station_cards(df_show, precio_col_label: str, station_coords: list, mis_paradas: list):
    ...
    for i, (_, row) in enumerate(df_cards.iterrows()):
        ...
        with st.container(border=True):
            ...
            with btn_cols[1]:
                if ya_en_plan:
                    st.button("✅ En plan", key=f"card_plan_{i}", disabled=True, use_container_width=True, help="Esta estación ya está añadida a tu plan.")
                else:
                    if st.button("➕ Añadir", key=f"card_add_{i}", use_container_width=True, type="primary", help="Añadir esta estación a tu plan."):
