    ...
    def render_trip_plan():
        if not st.session_state["mis_paradas"]:
            ...
            with c1, st.popover("🗑️ Vaciar Mi Plan", use_container_width=True):
                st.markdown("¿Estás seguro de que deseas eliminar todas las paradas?")
                if st.button("✔️ Sí, vaciar plan", type="primary", use_container_width=True, key="confirm_clear_plan"):
                    st.session_state["mis_paradas"] = []
                    st.rerun()
