...
def generate_map(
    ...
    for _, row in gdf_wgs84.iterrows():
        ...
        nombre = _esc(row.get("Rótulo"), "Sin nombre")
        municipio = _esc(row.get("Municipio"), "")
        provincia = _esc(row.get("Provincia"), "")
        direccion = _esc(row.get("Dirección"), "")
        horario = _esc(row.get("Horario"), "")
